...
def main(configuration_path, signal_path, background_path, predictions_path, model_path):
    '''
    Train a classifier on signal and background monte carlo data and write the model to
    MODEL_PATH in pmml or pickle format.
    ...
    print('Training classifier with {} protons and {} gammas'.format(num_protons,
                                                                     num_gammas))
    ...
    for fold, (train, test) in enumerate(tqdm(cv)):
        ...
        cv_predictions.append(pd.DataFrame({'label': ytest,
