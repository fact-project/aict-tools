...
def main(
    ...
    log.info(
        "Starting {} fold cross validation... ".format(model_config.n_cross_validations)
    )
    scores_disp = []
    scores_sign = []
    cv_predictions = []
    ...
    for fold, (train, test) in enumerate(
        ...
        cv_disp_train, cv_disp_test = (
            target_disp.values[train],
            target_disp.values[test],
        )
        cv_sign_train, cv_sign_test = (
            target_sign.values[train],
            target_sign.values[test],
        )
    ...
    log.info(
        "Mean R^2 score from CV: {:0.4f} ± {:0.4f}".format(
    ...
    log.info(
        "Mean accuracy from CV: {:0.4f} ± {:0.4f}".format(
